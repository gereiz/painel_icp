...
$config["QUERY_LOGIN_2"] = "SELECT * FROM accounts WHERE login = ? AND a.password = ?";
...
$config["QUERY_ITEMS_DETAILS_1"] = "SELECT i.*, (i.item_id) AS object_id, (i.item_type) AS item_id, (i.amount) AS count, (i.enchant) AS enchant_level, t.itemGrade FROM items AS i, icp_icons_interlude AS t WHERE i.item_type = t.itemId AND i.item_id = ? AND i.owner_id = ? AND (SELECT online FROM characters WHERE obj_Id = i.owner_id) = '0' AND (SELECT account_name FROM characters WHERE obj_Id = i.owner_id) = ?";
...
$config["QUERY_SELECT_CHARACTER_OFFLINE"] = "SELECT c.*, (SELECT level FROM character_subclasses WHERE char_obj_id = c.obj_Id AND isBase = '1') AS level FROM characters AS c WHERE c.obj_Id = ? AND c.account_name = ? AND c.online = '0'";
...
$config["QUERY_SELECT_ITEM_TO_ENCHANT"] = "SELECT i.*, (i.enchant) AS enchant_level, (SELECT itemGrade FROM icp_icons_interlude WHERE itemId = i.item_type) AS itemGrade FROM items AS i, characters AS c WHERE i.owner_id = c.obj_Id AND i.item_id = ? AND c.obj_Id = ? AND c.account_name = ?";
...
$config["QUERY_UPDATE_CHARACTER_OLYMPIADS"] = "UPDATE oly_nobles SET class_id = ?, points_current = '0', comp_done = '0', comp_win = '0', comp_loose = '0', comp_done_past = '0', points_past = '0', points_pre_past = '0', class_free_cnt = '0', class_based_cnt = '0', team_cnt = '0' WHERE char_id = ?";
...
$config["QUERY_RANKING_ADENA"] = "SELECT c.char_name, CASE WHEN (SELECT SUM(amount) FROM items WHERE owner_id = c.obj_Id AND item_id = '3470') > '0' THEN (SELECT SUM(amount) FROM items WHERE owner_id = c.obj_Id AND item_id = '3470') ELSE '0' END AS gold_bar, CASE WHEN (SELECT SUM(amount) FROM items WHERE owner_id = c.obj_Id AND item_id = '57' AND location = 'INVENTORY') > '0' THEN (SELECT SUM(amount) FROM items WHERE owner_id = c.obj_Id AND item_id = '57' AND location = 'INVENTORY') ELSE '0' END AS adena_inv, CASE WHEN (SELECT SUM(amount) FROM items WHERE owner_id = c.obj_Id AND item_id = '57' AND location = 'WAREHOUSE') > '0' THEN (SELECT SUM(amount) FROM items WHERE owner_id = c.obj_Id AND item_id = '57' AND location = 'WAREHOUSE') ELSE '0' END AS adena_war FROM characters AS c WHERE c.accesslevel = '0' ORDER BY gold_bar DESC, adena_inv DESC, adena_war DESC LIMIT {MAX_LIMIT}";
...
$config["QUERY_CHARACTER_STATUS_1"] = "SELECT c.*, ('0') AS charBroker, (c.obj_Id) AS char_id, (SELECT class_id FROM character_subclasses WHERE char_obj_id = c.obj_Id AND isBase = 1) AS base_class, (SELECT level FROM character_subclasses WHERE char_obj_id = c.obj_Id AND isBase = 1) AS level, IF((SELECT char_id FROM oly_nobles WHERE char_id = c.obj_Id) IS NULL, 0, 1) AS nobless FROM characters AS c WHERE c.obj_Id = ? AND c.account_name = ?";
$config["QUERY_CHARACTER_STATUS_2"] = "SELECT c.*, ('0') AS charBroker, (c.obj_Id) AS char_id, (SELECT class_id FROM character_subclasses WHERE char_obj_id = c.obj_Id AND isBase = 1) AS base_class, (SELECT level FROM character_subclasses WHERE char_obj_id = c.obj_Id AND isBase = 1) AS level, IF((SELECT char_id FROM oly_nobles WHERE char_id = c.obj_Id) IS NULL, 0, 1) AS nobless FROM characters AS c WHERE c.account_name = ?";
...
$config["QUERY_GET_CHARACTER_ITEMS_1"] = "SELECT *, (i.item_id) AS object_id, (i.item_type) AS item_id, (i.amount) AS count, (i.enchant) AS enchant_level, (SELECT char_name FROM characters WHERE obj_Id = i.owner_id) AS char_name FROM items AS i, icp_icons_interlude AS m WHERE i.item_type=m.itemId AND i.owner_id = ? AND (SELECT account_name FROM characters WHERE obj_Id = i.owner_id) = ? AND (SELECT online FROM characters WHERE obj_Id = i.owner_id) = '0' AND m.itemType IN('Armor','Weapon') AND i.location IN('PAPERDOLL','INVENTORY','WAREHOUSE') AND m.itemDrop='true' AND m.itemSell='true' AND m.itemTrade='true' AND enchant >= '0' AND itemGrade != ''{WHERE_PVP} AND i.location IN('{LOC}') ORDER BY m.itemId ASC";
$config["QUERY_GET_CHARACTER_ITEMS_2"] = "SELECT *, (i.item_id) AS object_id, (i.item_type) AS item_id, (i.amount) AS count, (i.enchant) AS enchant_level, (SELECT char_name FROM characters WHERE obj_Id = i.owner_id) AS char_name FROM items AS i, icp_icons_interlude AS m WHERE i.item_type = m.itemId AND i.owner_id = ? AND i.location IN('{LOC}') AND (SELECT account_name FROM characters WHERE obj_Id = i.owner_id) = ? ORDER BY m.itemId ASC";
